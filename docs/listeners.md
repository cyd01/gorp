...
#### Validate initial TCP bytes

TCP listeners can optionally validate the beginning of the application stream
before opening the backend tunnel. This works for plain TCP and for TLS-enabled
TCP listeners; with TLS, validation is performed after the downstream TLS
handshake on decrypted bytes. A failed check closes the connection.

`initial_bytes` is a position-based list. Each item describes one byte: use a
number for one accepted value, a one-byte character such as `"M"`, `*` for any
value, or a list for several accepted values. The list length determines how
many initial bytes are read.
The bytes are replayed to the backend, so stream protocols are not affected.
...
This example checks the first byte, ignores the second byte, and accepts only
`3`, `4`, or `5` at the third position.
...
All forward proxies can log incoming connections:
...
A `proxy_tls` listener provides the same forward-proxy behavior over TLS. The
client must trust the configured certificate (or its issuing CA).

```yaml
listeners:
  - name: outbound-proxy-tls
    type: proxy_tls
    address: ":3129"
    tls:
      cert_file: proxy.crt
...
Configure clients with `https://127.0.0.1:3129` as their proxy URL.

The `proxy` and `proxy_tls` listeners can require HTTP Basic authentication.
When `users` is set, both regular proxy requests and `CONNECT` requests must
include credentials. The value can be a list as shown below (a regular YAML
mapping is also accepted).

```yaml
listeners:
  - name: outbound-proxy-tls-with-auth
    type: proxy_tls
    address: ":3129"
    users:
      - alice: secret
...
A `socks5` listener accepts SOCKS5 `CONNECT` requests and tunnels them to the
requested destination. It does not use configured routes or backends. Without
`users`, it accepts connections without authentication. When `users` is set,
...
Configure clients with `socks5://127.0.0.1:1080` as their proxy URL. This is an
open proxy and should only be exposed to trusted networks.
