...
Paths can also be matched with wildcards. The pattern is evaluated against the
full request path, including the leading slash, and against the requested
filename. This allows patterns such as `*.mp4` to match files in any directory.
...
Path patterns support `*`, `?`, and character classes such as `[0-9]`, using
the same wildcard matching as host patterns. A route matches when its prefix,
one of its hosts, or one of its paths matches the request.
