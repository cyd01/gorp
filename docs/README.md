...
- [Middlewares](middlewares.md)
  ...
  - [x] OpenID Connect validation
  - [x] OpenAPI validation
  - [x] W2C distributed tracing
...
Usage of ./gorp:
  -config string
        configuration file
  -config-dir string
        directory containing YAML configuration fragments
```

The configuration source file is read from the `-config` command-line option or from one of these environment variables:
...
The configuration can also be split into multiple YAML files, in the same sub-directory. Just point the sub-directory with `-config-dir` parameter.
...
When the configuration source is a local file or a sub-directory, the configuration is reloaded automatically
