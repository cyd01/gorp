...
func (r *Router) ServeHTTPForEndpoint(w http.ResponseWriter, req *http.Request, endpoint string) {
	if route := r.routeFor(req, endpoint); route != nil {
...
func (r *Router) routeFor(req *http.Request, endpoint string) *Route {
	for _, route := range r.Routes {
		if route.MatchEndpoint(endpoint) && route.MatchPath(req) {
			return route
		}
	}
	for _, route := range r.Routes {
