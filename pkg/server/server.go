...
func (s *Server) startAdminListener(address, cert, key, ca, crl, ocspURL, minVersion string) error {
	srv, err := listener.NewAdminServer(address, cert, key, ca, crl, ocspURL, s.config, s.Shutdown, minVersion)
	if err != nil {
		return err
	}
...
func (s *Server) startListeners() error {
	for _, l := range s.config.Listeners {
		...
		case "proxy_tls":
			go s.runWithRecovery(fmt.Sprintf("TLS proxy '%s'", l.Name), func() error {
				return s.startProxyTLSListener(l)
			})

		case "socks5":
			go s.runWithRecovery(fmt.Sprintf("SOCKS5 '%s'", l.Name), func() error {
				return s.startSOCKS5Listener(l)
			})
...
func (l *limitedListener) TLSConfig() (*tls.Config, bool) {
	provider, ok := l.Listener.(interface{ TLSConfig() (*tls.Config, bool) })
	if !ok {
		return nil, false
	}
...
func (s *Server) startDynamicHTTPSListener(l config.Listener) error {
	...
	srv, err := listener.NewDynamicHTTPSServerWithClientCA(l.Address, l.TLS.CertFile, l.TLS.KeyFile, l.TLS.KeyPassphrase, l.TLS.CAFile, handler, l.TLS.MinVersion)
	if err != nil {
...
func (s *Server) startProxyListener(l config.Listener) error {
	var cascade *listener.CascadeProxy
	if l.Proxy != nil {
		cascade = &listener.CascadeProxy{URL: l.Proxy.URL, Username: l.Proxy.Username, Password: l.Proxy.Password}
	}
	handler, err := listener.NewProxyHandlerWithProxy(30*time.Second, l.Log, l.Users, cascade)
	if err != nil {
		return fmt.Errorf("failed to configure proxy cascade for '%s': %w", l.Name, err)
	}
	srv := &http.Server{
		Addr:    l.Address,
		Handler: handler,
...
func (s *Server) startProxyTLSListener(l config.Listener) error {
	var cascade *listener.CascadeProxy
	if l.Proxy != nil {
		cascade = &listener.CascadeProxy{URL: l.Proxy.URL, Username: l.Proxy.Username, Password: l.Proxy.Password}
	}
	srv, err := listener.NewProxyTLSServerWithProxy(l.Address, l.TLS.CertFile, l.TLS.KeyFile, 30*time.Second, l.Log, l.Users, cascade, l.TLS.MinVersion)
	if err != nil {
		return fmt.Errorf("failed to create TLS proxy server '%s': %w", l.Name, err)
	}
	if readHeaderTimeout, err := time.ParseDuration(l.ReadHeaderTimeout); err == nil && readHeaderTimeout > 0 {
		srv.ReadHeaderTimeout = readHeaderTimeout
	}
	...
	log.Printf("TLS proxy listener '%s' started on '%s'\n", l.Name, l.Address)
	if err := srv.ListenAndServeTLS("", ""); err != nil && err != http.ErrServerClosed {
		return fmt.Errorf("TLS proxy server '%s' error: %w", l.Name, err)
	}
	return nil
}

func (s *Server) startSOCKS5Listener(l config.Listener) error {
	ln, err := net.Listen("tcp", l.Address)
	if err != nil {
		return fmt.Errorf("listen for SOCKS5 server '%s': %w", l.Name, err)
	}
	if !s.registerTCPListener(ln) {
		return nil
	}
	log.Printf("SOCKS5 listener '%s' started on '%s'\n", l.Name, l.Address)
	if err := listener.HandleSOCKS5(s.ctx, ln, 30*time.Second, l.Log, l.Users); err != nil && err != context.Canceled {
		return fmt.Errorf("SOCKS5 server '%s' error: %w", l.Name, err)
	}
	return nil
}
...
func (s *Server) startTCPListener(l config.Listener) error {
	...
	if err := listener.HandleTCPListenerWithChecks(s.ctx, ln, l.Backends, l.InitialBytes, l.Routes); err != nil && err != context.Canceled {
