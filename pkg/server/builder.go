...
func buildMiddlewareForRoute(middlewareConfigs []config.MiddlewareConfig) ([]routing.Middleware, error) {
	...
	for _, mc := range middlewareConfigs {
		...
		case "correlation_id":
			uuidtype := helper.ParseStringOrDefault(mc.Config, "type", "")
			middlewares = append(middlewares, builtins.CorrelationID(uuidtype))

		case "distributed_tracing", "tracing", "trace_context":
			middlewares = append(middlewares, builtins.DistributedTracing())
		...
		case "rate", "rate_limit", "rate_limiter":
			...
			if extensions, err := helper.ParseStringSlice(mc.Config, "extensions"); err == nil && len(extensions) > 0 {
				middlewares = append(middlewares, builtins.RateLimitExtensions(maxReqs, window, extensions))
			} else {
		...
		case "openapi", "openapi_validate", "openapi_validation":
			filename := helper.ParseStringOrDefault(mc.Config, "file", "")
			middleware, err := routing.OpenAPIMiddleware(filename)
			if err != nil {
				return nil, fmt.Errorf("openapi middleware: %w", err)
			}
			middlewares = append(middlewares, middleware)
