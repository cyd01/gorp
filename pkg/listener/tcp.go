...
func StartTCP(address, cert, key, ca, crl, ocspURL string, backends []config.Backend, minVersion ...string) error {
	if len(backends) == 0 {
		log.Println("Warning: no backends configured for TCP listener")
	}

	listener, err := NewTCPListener(address, cert, key, ca, crl, ocspURL, minVersion...)
	if err != nil {
		return err
	}
...
func NewTCPListener(address, cert, key, ca, crl, ocspURL string, minVersion ...string) (net.Listener, error) {
...
func (l *tlsListener) TLSConfig() (*tls.Config, bool) {
...
func HandleTCPListener(ctx context.Context, listener net.Listener, backends []config.Backend, routeSets ...[]config.TCPRoute) error {
	return handleTCPListener(ctx, listener, backends, nil, routeSets...)
}

func HandleTCPListenerWithChecks(ctx context.Context, listener net.Listener, backends []config.Backend, checks []config.TCPByteCheck, routeSets ...[]config.TCPRoute) error {
	return handleTCPListener(ctx, listener, backends, checks, routeSets...)
}

func handleTCPListener(ctx context.Context, listener net.Listener, backends []config.Backend, checks []config.TCPByteCheck, routeSets ...[]config.TCPRoute) error {
	...
	for {
		...
		go handleTCPConnection(clientConn, backends, routes, listener, checks)
	}
}

func handleTCPConnection(clientConn net.Conn, backends []config.Backend, routes []config.TCPRoute, listener net.Listener, checks []config.TCPByteCheck) {
	defer func(conn net.Conn) {
		_ = conn.Close()
	}(clientConn)

	tlsProvider, isTLS := listener.(interface{ TLSConfig() (*tls.Config, bool) })
	var tlsConfig *tls.Config
	if isTLS {
		tlsConfig, isTLS = tlsProvider.TLSConfig()
	}
	if isTLS && tlsConfig != nil {
		...
		if selectedRoute != nil && selectedRoute.TLS != nil && selectedRoute.TLS.CertFile != "" && selectedRoute.TLS.KeyFile != "" {
			serverTLSConfig, err = helper.BuildTLSConfigForDownstreamWithMinVersion(selectedRoute.TLS.CertFile, selectedRoute.TLS.KeyFile, selectedRoute.TLS.CAFile, selectedRoute.TLS.CRLFile, selectedRoute.TLS.OCSPURL, selectedRoute.TLS.MinVersion)
			if err != nil {
	...
	if len(checks) > 0 {
		var err error
		clientConn, err = validateInitialBytes(clientConn, checks)
		if err != nil {
			log.Printf("Initial TCP byte check failed: %v", err)
			return
		}
	}
...
func validateInitialBytes(conn net.Conn, checks []config.TCPByteCheck) (net.Conn, error) {
	initial := make([]byte, len(checks))
	if _, err := io.ReadFull(conn, initial); err != nil {
		return nil, fmt.Errorf("read initial bytes: %w", err)
	}
	for index, check := range checks {
		...
		if !matched {
			return nil, fmt.Errorf("byte %d has value %d", index+1, initial[index])
		}
	}
	return &bufferedConn{Conn: conn, reader: io.MultiReader(bytes.NewReader(initial), conn)}, nil
}
