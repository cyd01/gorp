...
func BuildTLSConfigForDownstream(cert, key, ca, crl, ocspURL string) (*tls.Config, error) {
	return BuildTLSConfigForDownstreamWithMinVersion(cert, key, ca, crl, ocspURL, "")
}

func BuildTLSConfigForDownstreamWithMinVersion(cert, key, ca, crl, ocspURL, minVersion string) (*tls.Config, error) {
	tlsConfig := &tls.Config{}
	if minVersion != "" {
...
func ParseTLSVersion(value string) (uint16, error) {
	...
	default:
		return 0, fmt.Errorf("unsupported minimum TLS version %q (expected TLS1.0, TLS1.1, TLS1.2, or TLS1.3)", value)
	}
}

// downstream: clients -> reverse proxy
func BuildDynamicTLSConfig(caCertFile, caKeyFile, caKeyPassphrase string, minVersion ...string) (*tls.Config, error) {
	...
	var configuredMinVersion uint16 = tls.VersionTLS12
	if len(minVersion) > 0 && minVersion[0] != "" {
		configuredMinVersion, err = ParseTLSVersion(minVersion[0])
		if err != nil {
			return nil, err
		}
	}
...
func BuildDynamicTLSConfigWithClientCA(caCertFile, caKeyFile, caKeyPassphrase, clientCAFile string, minVersion ...string) (*tls.Config, error) {
	tlsConfig, err := BuildDynamicTLSConfig(caCertFile, caKeyFile, caKeyPassphrase, minVersion...)
	if err != nil || clientCAFile == "" {
		return tlsConfig, err
	}
	clientCA, err := ReadFile(clientCAFile)
	if err != nil {
		return nil, fmt.Errorf("read client CA certificate: %w", err)
	}
	clientCAPool := x509.NewCertPool()
	if !clientCAPool.AppendCertsFromPEM(clientCA) {
		return nil, fmt.Errorf("client CA file does not contain a certificate")
	}
