...
func TestParseTLSVersion(t *testing.T) {
	...
	for value, want := range tests {
		got, err := ParseTLSVersion(value)
		if err != nil {
			t.Fatalf("ParseTLSVersion(%q): %v", value, err)
		}
		if got != want {
			t.Errorf("ParseTLSVersion(%q) = %d, want %d", value, got, want)
		}
	}
	if _, err := ParseTLSVersion("TLS2.0"); err == nil {
		t.Fatal("ParseTLSVersion(TLS2.0) should fail")
	}
}
