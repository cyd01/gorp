...
func (l *SplitListener) Accept() (net.Conn, error) {
	...
	for {
		...
		if err != nil {
			// Stop only on fatal errors.
			if ne, ok := err.(net.Error); ok && ne.Temporary() {
				continue
			}
			// If the listener is closed, return the corresponding error.
		...
		if err != nil {
			...
			if errors.Is(err, io.EOF) ||
				...
				c.Close()
				continue // Ignore the error and wait for a new connection.
			}
			c.Close()
...
// ForceSecure forces the server to require TLS for all connections.
...
// SetTLS configures TLS with the given certificate and key files.
...
// WithoutHTTP1 disables HTTP/1.1 support on the server.
func (s *Server) WithoutHTTP1() *Server {
	s.SetHTTP1(false)
	return s
}

// WithoutHTTP2 disables HTTP/2 support on the server.
func (s *Server) WithoutHTTP2() *Server {
	s.SetHTTP2(false)
	return s
}

// WithoutUnencryptedHTTP2 disables unencrypted HTTP/2 support on the server.
...
// This method returns the underlying http.Server instance, allowing direct
// access to its fields and methods when needed.
